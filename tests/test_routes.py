...
class RoutesTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + \
            os.path.join(app.root_path, 'testdb')
    ...
    def test_del_method_not_allowed_on_rule(self):
        headers = {
            'Authorization': 'Basic %s' % b64encode(b"testuser:testpasswd").decode("ascii")
        }
    ...
    def test_invalid_rulename(self):
        ...
        response = self.app.get('/rastrea2r/api/v1.0/rule?rulename=invalidrule', headers=headers)
        self.assertIn(b'Not Found: description : Invalid Rule Requested', response.data)
        self.assertEqual(response.status_code, 404)
    ...
    def test_valid_rulename(self):
        ...
        response = self.app.get('/rastrea2r/api/v1.0/rule?rulename=example.yara', headers=headers)
    ...
    def test_post_result(self):
        ...
        response = self.app.post('/rastrea2r/api/v1.0/results', headers=headers, json=sample_data )
        self.assertIn(b'Results Saved Successfully', response.data)
        self.assertEqual(response.status_code, 200)
        response = self.app.post('/rastrea2r/api/v1.0/results', headers=headers, data="aa" )
