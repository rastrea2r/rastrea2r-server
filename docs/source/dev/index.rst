...
The rastrea2-server project implements a regression
...
not broken existing functionality. The rastrea2-server
...
``rastrea2r_server`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``rastrea2r_server`` package can be found.
